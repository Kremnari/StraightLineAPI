...
angular.module('ngIdle.idle', ['ngIdle.keepalive', 'ngIdle.localStorage'])
  .provider('Idle', function() {
    ...
    //ADDED
    var setName = this.name = function(name) {
        if(name==null) { name = ''; }
        options.name = name;
    }
    ...
    this.$get = ['$interval', '$log', '$rootScope', '$document', 'Keepalive', 'IdleLocalStorage', '$window',
      function($interval, $log, $rootScope, $document, Keepalive, LocalStorage, $window) {
        ...
        //ADJUSTED
        function getExpiry() {
          var obj = LocalStorage.get('expiry'+options.name);
        ...
        function setExpiry(date) {
          if (!date) LocalStorage.remove('expiry'+options.name);
          else LocalStorage.set('expiry'+options.name, {id: id, time: date});
        }
        ...
        var svc = {
          ...
          setName: function(name) {
              changeOption(this, setName, name);
          },
